...
get_pdb_essentials_for = function(dist,pdb,sepn="[-]"){
	...
	#res$xyz = mxyz[filter,]
	res$xyz = pdb$atom[filter,9:11] # PERIGO!
...
download_pdb = function(PDBS,pdbpath,sufix=".pdb", destfile){
  #browser()
...
make_pdb_title = function(title,size=80){
  header = "HEADER    PROTEIN                                                               \n"
  title = paste0("TITLE     ",title)
  title = str_trunc(title,size,"right")
  title = paste0(header,title,"\n")
  return(title)
  #x = str_extract_all(title, paste0('.{1,',size,'}'))
  #browser()
}

add_conect_tags = function(pdb,pdbpath,sufix=".pdb",pre.sufix,tag="CONECT",tempath="temp/"){
  
  filename = paste0(tempath,pdb$pdbname,sufix)
  #browser()
  if (file.exists(filename)){
    tar.tags = read_lines(filename)
    tar.tags = read_lines(filename,n_max=(length(tar.tags)-1) )
    #filenameori = filename %>% str_replace(paste0("2",pre.sufix),"")
    filenameori = paste0(pdbpath,pdb$pdbname,sufix)
    #browser()
    if (file.exists(filenameori)){
      all.tags = read_lines(filenameori)
      ids = grep(tag,all.tags)
      if (length(ids)){
        con.tags = all.tags[ids]
        val.tags = c()
        for (i in 1:length(con.tags)){
          n = parse_number(con.tags[i])
          if (n %in% pdb$atom$eleno){
            val.tags = c(val.tags,con.tags[i])
          }
        }
        tar.tags = c(tar.tags,val.tags,"END")
        print(paste("Adding CONECT tags to",filename))
        write_lines(tar.tags,filename)
        #write_lines(val.tags,filename,append=T)
        #browser()
      } else {
        print(paste("WARNING:", filename,"for PDB",pdb$pdbname,"has no CONECT tags"))
      }
    } else {
      print(paste("WARNING:", filename,"for PDB",pdb$pdbname,"doesnt exist for conect adding and that's weird!"))
    }
  } else {
    print(paste("WARNING:", filename,"for PDB",pdb$pdbname,"doesnt exist for conect adding and that's strange!"))
  }
}

write_pdb = function(pdb,pdbpath,force=F,sufix=".pdb",title=F,
                     conect_tag=F,pdb_out=F,pre.sufix="",pdbpathout="pdb-out/",tempath="temp/"){
	
	#res = c()
  if (!dir.exists(tempath)){
    dir.create(tempath)
  }
  ...
  if (pdb_out){
	...
	#browser()
	if(conect_tag){
	  add_conect_tags(pdb,pdbpath,sufix=sufix,pre.sufix=pre.sufix,tempath=tempath)
	}
	if(title){
	  print(paste("WRITING PDB OUTPUT AT",filename))
	  filetitle = paste0(pdbpath,"temp.tit")
	  title = make_pdb_title(pdb$title)
	  cat(title,file=filetitle)
	  command = paste0("cat ",filename," >> ",filetitle)
	  try(system(command))
	  command = paste0("mv ",filetitle," ",filename)
	  try(system(command))
	}
	if(pdb_out){
	  print(paste("MOVING PDB OUTPUT TO",pdbpathout))
	  command = paste0("mv ",filename,paste0(" ",pdbpathout)) # pdb-out/")
	  try(system(command))
	  #browser()
	  pdb$filename = filename %>% str_replace(tempath,pdbpathout)
	  #pdb$pdbname = paste0(pdb$pdbname,pre.sufix)
	  return(pdb)
	}
	#return(res)
}
...
verify_atomname_nonconformity = function(pdb,ligs){
  
  #pdb$ligands = pdb$ligands %>% filter(resname %in% ligs$resname, resid %in% ligs$resid)
  for (i in 1:dim(ligs)[1]){
    f = pdb$ligands$resname == ligs$resname[i] & pdb$ligands$resid == ligs$resid[i]
    ids = which(table(pdb$ligands$elename[f])>1)
    #browser()
    if (length(ids)){
      pdb$ligands$elename[f] = paste0(pdb$ligands$elename[f],pdb$ligands$eleid[f])
      f = pdb$atom$resid == ligs$resname[i] & pdb$atom$resno == ligs$resid[i]
      pdb$atom$elety[f] = paste0(pdb$atom$elety[f],pdb$atom$eleno[f])
    }
  }
  #browser()
  return(pdb)
...
mol_list_to_table = function(mol){
  mol = t(unlist(str_split(mol,",")))
  mol.n = length(mol)
  mol.col = paste(sprintf("A%s B%s",1:(mol.n/2),1:(mol.n/2)),collapse = " ")
  mol.col = unlist(str_split(mol.col," "))
  colnames(mol) = mol.col
  mol = as_tibble(mol)
  mol = mol %>% pivot_longer(everything(),
                               names_to = c(".value", "set"),
                               names_pattern = "(.)(.)"
  )
  mol = mol %>% rename(resname = A, resid = B)
  mol = mol %>% mutate(chainid = LETTERS[2:(n()+1)])
  return(mol)
}

select_pdb_mols = function(pdb,mols,also_chain=F){
  
  selni = c()
  for (i in 1:dim(mols)[1]){
    #browser()
    selresn = atom.select(pdb,resid=mols$resname[i])
    selresi = atom.select(pdb,resno=mols$resid[i])
    selresni =  combine.select(selresn,selresi,operator="AND")
    if (also_chain){
      selchain = atom.select(pdb,chain=mols$chainid)
      selresni =  combine.select(selresni,selchain,operator="AND")
    }
    if (i == 1){
      selni = selresni
    } else {
      selni = combine.select(selni,selresni,operator="OR")
    }
  }
  return(selni)
...
TRY_filter_one_chain_keeping_ligs = function(pdb,ligs,tars,water="HOH",sep=",",unify_chain=F,ref_chain="A"){
  
  #browser()
  #print(ligs)
  
  if(unify_chain){
    pdb$atom$chain = ref_chain
    pdb$ligands$chainid = ref_chain
  }
  
  ligs = mol_list_to_table(ligs)
  if (!is.null(tars)){
    tars = mol_list_to_table(tars)
    tars$chainid = "A"
  }

  nchains = pdb$ligands %>% group_by(chainid) %>% summarise(n=n())
  
  if (dim(nchains)[1]>1){
    print(paste("WARNING: PDB",pdb$pdbname,
                "has more than 2 chains for ligands. Choosing the first..."))
    pdb$ligands = pdb$ligands %>% filter(resname %in% ligs$resname, 
                                         resid %in% ligs$resid,
                                         chainid == nchains$chainid[1])  
  } else {
    pdb$ligands = pdb$ligands %>% filter(resname %in% ligs$resname, resid %in% ligs$resid)
  }
  #browser()
  
  auxall = tibble()#;browser()
  for (i in 1:dim(ligs)[1]){
    aux = pdb$ligands %>% filter(resname == ligs$resname[i],resid == ligs$resid[i])
    aux$chainid = ligs$chainid[i]
    auxall = bind_rows(auxall,aux)
  }
  pdb$ligands = auxall
  
  ligs = ligs %>% mutate(across(resid, as.numeric))
  
  if(!is.null(tars)){
    tars = tars %>% mutate(across(resid, as.numeric))
  }
  
  nchains = pdb$atom %>% filter(type=="ATOM") %>%  group_by(chain) %>% summarise(n=n())
  
  #browser()
  
  #garante todas as cadeias alvo "A"
  if (dim(nchains)[1]==1){
    f = pdb$atom$type == "ATOM" | pdb$atom$resid == water
    pdb$atom$chain[f] = LETTERS[1]
    if (!is.null(tars)){
      f = pdb$atom$resid == tars$resname & pdb$atom$resno == tars$resid
      pdb$atom$chain[f] = LETTERS[1]
    }
  } else if ( !(LETTERS[1] %in% nchains$chain) ){
    print(paste("WARNING: PDB",pdb$pdbname,"has more than 2 chains and no chain A. Choosing the first..."))
    f = (pdb$atom$type == "ATOM" | pdb$atom$resid == water) & pdb$atom$chain == nchains$chain[1]
    pdb$atom$chain[f] = LETTERS[1]
    if (!is.null(tars)){
      f = pdb$atom$resid == tars$resname & pdb$atom$resno == tars$resid & pdb$atom$chain == nchains$chain[1]
      pdb$atom$chain[f] = LETTERS[1]
    } #else if (!is.null(tars)){
      #f = pdb$atom$resid == tars$resname & pdb$atom$resno == tars$resid & pdb$atom$chain == tars$chainid[1]
      #pdb$atom$chain[f] = LETTERS[1]
    #}
  }
  ...
  sel1 = atom.select(pdb,resid=water)
  sel2 = atom.select(pdb,chain=LETTERS[1])
  sel12 = combine.select(sel1,sel2,operator="AND")
  
  sel3 = atom.select(pdb,type="ATOM")
  sel4 = atom.select(pdb,chain=LETTERS[1])
  sel34 = combine.select(sel3,sel4,operator="AND")
  
  sel = combine.select(sel12,sel34,operator="OR") #across
  #browser()
  if (!is.null(tars)){
    sel.tars = select_pdb_mols(pdb,tars,also_chain=T)
    sel = combine.select(sel,sel.tars,operator="OR")
  }
  
  # if (!is.null(tars)){
  #   sel.tars = select_pdb_mols(pdb,tars)
  # }
  
  sel.ligs = select_pdb_mols(pdb,ligs)
  sel = combine.select(sel,sel.ligs,operator="OR")
  
  #selni = c()
  # for (i in 1:dim(ligs)[1]){
  #   #browser()
  #   selresn = atom.select(pdb,resid=ligs$resname[i])
  #   selresi = atom.select(pdb,resno=ligs$resid[i])
  #   selresni =  combine.select(selresn,selresi,operator="AND")
  #   if (i == 1){
  #     selni = selresni
  #   } else {
  ...
  #   }
  # }
  #sel = combine.select(sel,selni,operator="OR")
  
  pdb$atom = pdb$atom[sel$atom,]
  pdb$xyz = pdb$xyz[sel$xyz]
  pdb$calpha = pdb$atom$elety == "CA"

  #;browser()
  auxall = pdb$atom %>% filter(type == "ATOM",chain =="A")
  for (i in 1:dim(ligs)[1]){
    f = pdb$atom$resid == ligs$resname[i] & pdb$atom$resno == ligs$resid[i]
    pdb$atom$chain[f] = ligs$chainid[i]
  }
  #names(chain.def) = c("C","C")
  pdb$chains = c("A",ligs$chainid)
  names(pdb$chains) = rep("C",length(pdb$chains))
  pdb$ligands = as.data.frame(pdb$ligands)
  #browser()
  
  pdb = verify_atomname_nonconformity(pdb,ligs)
...
#"pymol -cp Pymol/script-python-v1.py  --  pdb/CLSEPECL4SA1.pdb pdb/CLSEPECL4SA1.pdb 10"
filter_one_chain_keeping_ligs = function(pdb,ligs,tars,water="HOH",sep=",",unify_chain=""){
  
  #browser()
  #print(ligs)
  if (water=="NO"){
    sel0 = atom.select(pdb,resid=water,inverse=T)
    pdb$atom = pdb$atom[sel0$atom,]
    pdb$xyz = pdb$xyz[sel0$xyz]
    pdb$calpha = pdb$atom$elety == "CA"
  }
  if(unify_chain!=""){
    pdb$atom$chain = unify_chain
    pdb$ligands$chainid = unify_chain
  }
...
filter_one_chain = function(pdb,chain.def = c("A","C"),water="HOH"){
  
  lig = pdb$ligands %>% filter(resname != water) %>% group_by(chainid) %>% summarise(n=n())
  nchains = dim(lig)[1]
  pdb$one = F
  #browser()
  if (nchains==2){
    chain1 = lig$chainid[1]
    sel = atom.select(pdb,chain=chain1)
    pdb$atom = pdb$atom[sel$atom,]
    pdb$xyz = pdb$xyz[sel$xyz]
    pdb$calpha = pdb$atom$elety == "CA"
    pdb$ligands = pdb$ligands %>% filter(chainid == chain1)
    
    pdb$atom = pdb$atom %>% mutate(chain = case_when(
                                        type == "HETATM" & resid != water ~ chain.def[2],
                                        TRUE ~ chain.def[1]
                                  ))
    pdb$ligands = pdb$ligands %>% mutate(chainid = case_when(
                                            resname != water ~ chain.def[2],
                                            TRUE ~ chain.def[1]
                                        ))
    pdb$one = T
    
    names(chain.def) = names(pdb$chains)
    pdb$chains = chain.def
    #browser()
    
  } else if (nchains == 1) {
    #browser()
    atom = pdb$atom %>% filter(type=="ATOM") %>%  group_by(chain) %>% summarise(n=n())
    if (atom$chain[1]==lig$chainid){ ### atom$chain pode ter mais de uma cadeia...
      lig.table = pdb$ligands %>% filter(resname != water) %>% group_by(resname,resid) %>% 
        summarise(n=n()) %>% ungroup()
      lig = lig.table %>% filter(n==max(n)) %>% select(resname)#;browser()
      pdb$ligands = pdb$ligands %>% filter(resname %in% c(water,lig$resname))
      
      sel1 = atom.select(pdb,type="ATOM")
      sel2 = atom.select(pdb,resid=water)
      sel3 = atom.select(pdb,resid=lig$resname)
      sel = combine.select(sel1,sel2,sel3,operator="OR")
      #y = pdb$atom[sel$atom,]
      pdb$atom = pdb$atom[sel$atom,]
      pdb$xyz = pdb$xyz[sel$xyz]
      pdb$calpha = pdb$atom$elety == "CA"
      #x = pdb$atom %>% filter(type=="ATOM" | resid %in% c(water,lig$resname))
      #browser()
      pdb$atom = pdb$atom %>% mutate(chain = case_when(
        type == "HETATM" & resid != water ~ chain.def[2],
        TRUE ~ chain.def[1]
      ...
      print(paste("OPS... nothing to do with filter_one_chain for",pdb$pdbname))
    }
  } else {
    print(paste("WARNING: number of lig chains is bigger than expected for",pdb$pdbname,":",nchains))
    print(paste("Impossible to filter one chain!"))
  }
  #View(pdb$atom)
  #View(pdb$ligands)
  #browser()
...
#1)https://stackoverflow.com/questions/56636417/bunch-recoding-of-variables-in-the-tidyverse-functional-meta-programing
#https://www.rebeccabarter.com/blog/2019-08-19_purrr/
#https://stackoverflow.com/questions/53464733/select-variables-from-list-of-tibbles-using-names-in-each-tibble

 
make_res_decomposition = function(pdb, dec.table){
  
  
  res.code = dec.table %>% split(.$atom) %>% map(function(tbl){select(tbl,res2)$res2})
  pdb$atom = pdb$atom %>% mutate(temp = recode(elety, !!!res.code))
  f = pdb$atom$resid == dec.table$res1[1]
  pdb$atom$resid[f] = pdb$atom$temp[f]
  pdb$atom$temp = NULL
  ### OLHAR 1)
  #browser()
  return(pdb)
...
initialize_db_list_for = function(pdb_name,pdbpath,meso,aacode,nmax.atom,workpath,prodpath,ipaddr,ncores,rm.h=T,
                                  sufix=".pdb",conform_1_chain=F,ligs=NULL,tars=NULL,slim=NULL,pre.sufix="",
                                  pdbpathout="pdb-out/",dec.table=NULL,unify_chain="",water="HOH"){
  ...
  #pdb <- read.pdb(file,maxlines = -1, multi = FALSE, rm.insert = FALSE, rm.alt = TRUE, ATOM.only = FALSE, verbose = TRUE)
	...
	#print(pdb_name)
  #browser()
	pdb = make_it_mass_pdb_reading(pdb_name,pdbpath,nmax.atom=nmax.atom,sufix=sufix,slim=slim)
	#return(pdb)
	...
	if (conform_1_chain){
	  if(is.null(ligs)){
	    pdb = filter_one_chain(pdb)#;browser()
	  } else {
	    pdb = filter_one_chain_keeping_ligs(pdb,ligs=ligs,tars=tars,
	                                        unify_chain=unify_chain,water=water)
	    #browser()
	  }
	  if(!is.null(dec.table)){
	    pdb = make_res_decomposition(pdb,dec.table)
	  }
	  #browser()
	  pdb = write_pdb(pdb,pdbpath,force=T,sufix=sufix,pre.sufix=pre.sufix,
	            title=T,conect_tag=T,pdb_out=T,pdbpathout=pdbpathout)
	  #add_conect_tags(pdb,pdbpath,sufix="a.pdb")
	  return(pdb)
	}
	#;print("0")
  #browser()
...
initialize_db_list = function(PDBS,pdbpath,meso,aacode,nmax.atom,workpath,prodpath,ipaddr,vcores=c(0,0),
                              pre.sufix="",sufix=".pdb",conform_1_chain=F,ligs=NULL,tars=NULL,slim=NULL,
                              pdbpathout="pdb-out/",dec.table=NULL,unify_chain = "",water="HOH"){
	...
	if (vcores[1]){
		...
		db = foreach(i=1:tam) %dopar% {
			initialize_db_list_for(PDBS[i],pdbpath,meso,aacode,nmax.atom,workpath,prodpath,ipaddr,vcores[2],
			                       pre.sufix=pre.sufix,sufix=sufix,conform_1_chain=conform_1_chain,
			                       ligs=ligs[i],tars=tars[i],slim=slim,pdbpathout=pdbpathout,
			                       dec.table=dec.table,unify_chain=unify_chain,water=water)
		}
	}else{
		print(paste("Doing it sequential..."))
		for(i in 1:tam){
			db[[i]] = initialize_db_list_for(PDBS[i],pdbpath,meso,aacode,nmax.atom,workpath,prodpath,ipaddr,vcores[2],
			                                 pre.sufix=pre.sufix,sufix=sufix,conform_1_chain=conform_1_chain,
			                                 ligs=ligs[i],tars=tars[i],slim=slim,pdbpathout=pdbpathout,
			                                 dec.table=dec.table,unify_chain=unify_chain,water=water)	
		}
...
align_db_by_rot = function(pdb.target,pdb.mobile,pdbpath,workpath,prodpath,vcores,
                             pre.sufix,sufix,pdbpathout,command,window=10,k=3){
  
  pdb.mobile$align$rmsd = -1
  rot = pdb.target$align$rot
  pdb.mobile$align$rot = rot
  xyz = matrix(pdb.mobile$xyz,ncol=3,byrow=T)
  xyz = transform_by_rot_matrix_ce(xyz,rot)
  #xyz = transform_by_rot_matrix(xyz,rot)
  pdb.mobile$xyz = as.vector(t(xyz))
  #browser()
  x = write_pdb(pdb.mobile,pdbpath,force=T,sufix=sufix,pre.sufix=pre.sufix,
                title=T,conect_tag=T,pdb_out=T,pdbpathout=pdbpathout)
  return(pdb.mobile)
}

align_db_list_for = function(pdb.target,pdb.mobile,pdbpath,workpath,prodpath,vcores,
                             pre.sufix,sufix,pdbpathout,command,ref.rot="by_script",i,ids,window=10,k=3){
  
#pymol -cp Pymol/script-python-v1.py -- Pymol/A.pdb Pymol/BB.pdb 10 
  
  
  #browser()
  if (i %in% ids){
    pdb.mobile$align = list()  
    if (ref.rot=="by_script"){
      print(paste("ALIGNING by file script:",pdb.target$pdbname,pdb.mobile$pdbname))
      args = paste(pdb.target$filename,pdb.mobile$filename,window)
      pdb.mobile$align$files = c(pdb.target$filename,pdb.mobile$filename)
      pdb.mobile$align$entities = c(pdb.target$pdbname,pdb.mobile$pdbname)
      #comout = system2(command,args=args,stdout=T)
      print(paste(command,args))
      comout = try(system(paste(command,args),intern=T))
      print(comout)
      # x = system2(command = "pymol", args = "-cp Pymol/script-python-v1.py  --  pdb/7NF5.pdb pdb/7NF5.pdb 10", stdout = T)
      # print(x)
      id1 = grep("RMSD",comout)
      id2 = grep("^\\[",comout)
      id3 = grep("\\]$",comout)
      #browser()
      #rot = parse_number(comout[id2:id3])
      #matrix(dball[[1]]$align[[51]]$rot,ncol=4,byrow=T)
      #transform_by_rot_matrix = function(xyz, rot)
      
      if (length(id1)&length(id2)&length(id3)){
        pdb.mobile$align$rmsd = comout[id1]
        rot = round(parse_number(comout[id2:id3]),k)
        pdb.mobile$align$rot = rot
        xyz = matrix(pdb.mobile$xyz,ncol=3,byrow=T)
        xyz = transform_by_rot_matrix_ce(xyz,rot)
        #xyz = transform_by_rot_matrix(xyz,rot)
        ...
        x = write_pdb(pdb.mobile,pdbpath,force=T,sufix=sufix,pre.sufix=pre.sufix,
                        title=T,conect_tag=T,pdb_out=T,pdbpathout=pdbpathout)
        #browser()
        #transform_by_rot_matrix_ce
        #transform_by_rot_matrix()
      } else {
        print(paste("WARNING: failed pdb alignment for ",pdb.target$pdbname,pdb.mobile$pdbname))
        pdb.mobile$align$rmsd = -2
        pdb.mobile$align$rot = 0
      } 
    
    } else {
      #browser()
      print(paste("ALIGNING by rotation matrix:",pdb.target$pdbname,pdb.mobile$pdbname))
      pdb.mobile$align$rmsd = -1
      rot = pdb.target$align$rot 
      pdb.mobile$align$rot = rot
      xyz = matrix(pdb.mobile$xyz,ncol=3,byrow=T)
...
align_db_list = function(db,ids,pdbpath,workpath,prodpath,vcores=c(0,0),pre.sufix="",sufix=".pdb",
                         pdbpathout="pdb-out/",iref=rep(1,length(db))){
  ...
  command = paste(command,flags[1],script,flags[2])
  #browser()
  #db.len = length(db)
  #tam = c(1,rep(1:2,db.len))
  #tam = tam[1:db.len]
  tam = length(db)
#  if (tam>1){
    if (vcores[1]){
      ...
      registerDoMC(vcores[1])
      db = foreach(i=1:tam) %dopar% {
        align_db_list_for(db[[iref[i]]],db[[i]],pdbpath,workpath,prodpath,vcores[2], pre.sufix=pre.sufix,
                          sufix=sufix,pdbpathout=pdbpathout,command=command,#ref.rot=names(iref[i]),
                          i=i,ids=ids)
      }
    }else{
      print(paste("Doing it sequential..."))
      for(i in 1:tam){
        db[[i]] = align_db_list_for(db[[iref[i]]],db[[i]],pdbpath,workpath,prodpath,vcores[2], pre.sufix=pre.sufix,
                                    sufix=sufix,pdbpathout=pdbpathout,command=command,#ref.rot=names(iref[i]),
                                    i=i,ids=ids)
      }
    }
#  } else {
#    print(paste("WARNING: nothing to align over. There is only one PDB in list"))
#  }
  return(db)
}
