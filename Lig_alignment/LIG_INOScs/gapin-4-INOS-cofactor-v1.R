...
if(ii){
  ...
  filename = "LIGS/leo-ligs-INOScs-v1.csv"
...
if(ii){
  ...
  pdb = initialize_db_list(PDBS,pdbpathin,meso,aacode,nmax.atom,workpath,prodpath,ipaddr,
                          ...
                          dec.table=dec.table,unify_chain="A",water="NO")
...
if (0){
  ...
  #iref = c(1,1,rep(2:1,length(pdb)))
  #names(iref) = c(rep("T",2),rep("F",length(iref)-2))
  #iref = c(1,1,rep(2:1,length(pdb)))
  #iref = iref[1:length(pdb)]
  iref = c(1,1,2,1,rep(4,length(pdb)-4));names(iref)=c(rep("by_script",4),rep("by_rot",length(iref)-4))
  ids = 1:2
  pdb = align_db_list(pdb,ids,pdbpath,workpath,prodpath,vcores=c(ncores=0,ncores=0),pre.sufix=pre.sufix,
                     sufix=pdb.sufix,pdbpathout="pdb-align/",iref=iref)
  #iref = c(rep(2:1,length(pdb)-2));names(iref)="by_rot"
  ids = 3:length(pdb)
  pdb = align_db_list(pdb,ids,pdbpath,workpath,prodpath,vcores=c(ncores=0,ncores=0),pre.sufix=pre.sufix,
                      sufix=pdb.sufix,pdbpathout="pdb-align/",iref=iref)
...
if(1){
  ...
  seq.id = c(6,8) #crystal
  #seq.id = c(52,54,56,58) #Docked-trans-Geranylcetone-a1-iNOScs-Pose1 a 4
  #seq.id = c(66,68,70) #Docked-cis-geranylcetone-iNOScs-Pose1 a 3
  #seq.id = c(seq(16,by=2,58)) #iNOScs-and-iNOSas
  m.inosfac.crys = consolidate_res_table(dba2,seq.id,noid=c("HCC","HCO","HCR","HFN"))
  seq.id = c(10,12,14) # redocking sapropterin
  #seq.id = c(66,68,70) #Docked-cis-geranylcetone-iNOScs-Pose1 a 3
  #seq.id = c(52,54,56,58) #Docked-trans-Geranylcetone-a1-iNOScs-Pose1 a 4
  #seq.id = c(seq(60,by=2,82)) #Just-iNOScs
  m.inosfac.ctrl = consolidate_res_table(dba2,seq.id,noid=c("HCC","HCO","HCR","HFN"))
  seq.id = c(seq(16,by=2,length(dba2[[1]]))) #all remaning 
  #seq.id = c(seq(84,by=2,length(dba2[[1]]))) #just-iNOScs-WORSE-to-iNOSas-COX1
  m.inosfac.ligs = consolidate_res_table(dba2,seq.id,noid=c("HCC","HCO","HCR","HFN"))
  #seq.id = c(2,4)
  #m.inosfac.cry24 = consolidate_res_table(dba2,seq.id,noid=c("HCC","HCO","HCR","HFN"))
  dba2.size = object.size(dba2)
...
if(1){
  ...
  p.inosfac.crys = plot_lig_res_interactions(m.inosfac.crys,scene="scene1",area.lim=100,alpha=0.1,n=1200)
  p.inosfac.ctrl = plot_lig_res_interactions(m.inosfac.ctrl,scene="scene2",area.lim=100,alpha=0.1,n=1200)
  p.inosfac.ligs = plot_lig_res_interactions(m.inosfac.ligs,scene="scene3",area.lim=100,alpha=0.1,n=600)
  #p.inosfac.cry24 = plot_lig_res_interactions(m.inosfac.cry24,scene="scene3",area.lim=100,alpha=0.1,n=800)
  #p.all = subplot(p.inosfac.crys,p.inosfac.ctrl,p.inosfac.ligs);print(p.all)
  #p.all = plotly_all_together_INOScs_ligs(p.inosfac.crys,p.inosfac.ctrl,p.inosfac.ligs);print(p.all)
  range = c(2,18, 90,104, 4, 22)
  text = c("Crystal iNOScs Sapropterin (A/B)",
           "Docked iNOScs Sapropterin poses",
           "Top docked iNOScs poses")
  # text = c("Docked trans-Geranylcetone iNOScs Poses",
  #          "Docked cis-geranylcetone iNOScs Poses",
  #          "")
  # text = c("Docked cis-Geranylcetone iNOScs Poses",
  #          "Docked trans-geranylcetone iNOScs Poses",
  #          "")
  # text = c("iNOScs and iNOSas Poses",
  #          "Just iNOScs Poses",
  #          "just iNOScs WORSE to iNOSas-COX1")
  p.all = plotly_all_together_ligs(p.inosfac.crys,p.inosfac.ctrl,p.inosfac.ligs,text=text,range=range)
  #p.all = plotly_all_together_ligs(p.inosfac.crys,p.inosfac.ctrl,text=text,range=range)
  #p.all = plotly_all_together_ligs(p.inosfac.crys,p.inosfac.ctrl,text=text,range=range)
  print(p.all)
  #print(p.inosfac.cry)
  dba2.size = object.size(dba2)
...
if(1){
  library(orca)
  p.sizes = c(1200,600)
  #orca(p.all,"../Images/iNOScs-bygroup-v2.png",scale=2,width=p.sizes[1],height=p.sizes[2])
  # orca(p.all,"../Images/cis-trans-Geranylcetone-iNOScs-v2a.png",scale=2,
  #      width=p.sizes[1],height=p.sizes[2])
  # orca(p.all,"../Images/cis-trans-Geranylcetone-iNOScs-v2b.png",scale=2,
  #      width=p.sizes[1],height=p.sizes[2])
  orca(p.all,"../Images/iNOScs-Sapropterin-Top-Ligs-v2.png",scale=2,
       width=p.sizes[1],height=p.sizes[2])
}
