...
if(0){
  ...
  filename = "LIGS/leo-ligs-cox1-v3.csv" ### ALTEROU
...
if(1){
  ...
  seq.id = c(80,82)
  seq.id = c(seq(10,by=2,14)) # a-Caryophillene-COX-1
  seq.id = c(seq.id,seq(34,by=2,38)) # b-Caryophillene-COX-1
  seq.id = c(seq.id,seq(70,by=2,74)) # d-Caryophillene-COX-1
  seq.id = c(seq.id,seq(76,by=2,80)) # e-Caryophillene-COX-1
  seq.id = c(seq.id,seq(82,by=2,86)) # g-Caryophillene-COX-1
  seq.id = c(2)
...
if(1){
  ...
  p.ligs2 = plot_lig_res_interactions(m.ligs2,scene="scene2",area.lim=100,alpha=0.1,n=1000)
  p.ligs3 = plot_lig_res_interactions(m.ligs3,scene="scene3",area.lim=100,alpha=0.1,n=500)
  
  #range = c(-8, 8,90, 110,10, 30)
  range = c(240, 260,80, 120,-48, -28)
  text = c("Docked Caryophillene COX1 Poses",
           "Docked Caryophillene COX1 Poses",
           "")
  text = c("Arachidonic Acid Crystal COX1",
           "Docked Arachidonic Acid COX1 Poses",
           "Docked Aroeira Ligand COX1 Poses")
  p.sizes = c(3200,1800)
  p.sizes = c(1200,600)
  p.all = plotly_all_together_ligs(p.ligs1,p.ligs2,p.ligs3,text=text,range=range)
  #p.all = plotly_all_together_ligs(p.ligs1,p.ligs2,text=text,range=range,p.sizes=p.sizes)
  print(p.all)
...
if(1){
  library(orca)
  orca(p.all,"../Images/Arachidonic-Aroeira-COX1-v2.png",scale=2,width=p.sizes[1],height=p.sizes[2])
}

#########################  OLD #################
# CONSOLIDATE RES.TABLE
if(0){
  print(taskline)
  ttime = c(proc.time()[3],ttime);print("CONSOLIDATE RES TABLE...")
